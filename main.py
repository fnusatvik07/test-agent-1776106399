# ui_user_6ee475e4/main.py
"""
Test Agent - Auto-generated FastAPI service
Role: General-purpose question answering assistant
"""
import asyncio
import json
import os
...
app = FastAPI(
    title="Test Agent",
    description="General-purpose question answering assistant",
    version="1.0.0"
)
...
CORS_ORIGINS = [
    ...
    f"http://localhost:8043",
    f"http://localhost:8044",
    f"http://127.0.0.1:8042",
    f"http://127.0.0.1:8043",
    f"http://127.0.0.1:8044"
]
...
AGENT_CONFIG = {
    ...
    "system_prompt": """You are a professional test agent designed to answer questions accurately and thoroughly.

Core Responsibilities:
- Answer user questions with clarity, precision, and professionalism
- Research information when needed to provide accurate and up-to-date responses
- Ask clarifying questions when a query is ambiguous or requires additional context

Behavior Guidelines:
- Maintain a professional tone at all times
- Provide concise yet comprehensive answers
- Cite sources when referencing external information
- If you are unsure about something, acknowledge the limitation and research further rather than guessing
- Structure responses clearly using appropriate formatting when it aids readability

Workflow:
1. Analyze the user's question to understand what is being asked
2. If the question is unclear, ask a clarifying question before proceeding
3. If you need current or specific information, use web search and fetch tools to research
4. Synthesize the information and provide a well-structured answer
5. Offer to elaborate or answer follow-up questions as needed""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8041",
        "frontend_url": f"http://localhost:8042",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8041")
    print(f"[FRONTEND] Frontend URL: http://localhost:8042")
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8041/docs")
    print(f"[HEALTH] Health: http://localhost:8041/health")
